...
(function () {
  ...
  // Pre-apply any URL filters on load (server already rendered the filtered
  // state, but this keeps DOM state in sync with the URL).
  var initial = current();
  if (initial.q || initial.section || initial.status !== 'all') {
    apply(initial.q, initial.section, initial.status);
  }
})();
